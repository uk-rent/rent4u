...
import { Heart } from "lucide-react"; // Añadimos icono de corazón de Lucide
...
interface Property {
  ...
  available: string;
  featured?: boolean;
  isFavorite?: boolean; // Añadimos esta propiedad
}

interface PropertyCardProps {
  property: Property;
  onFavoriteToggle: (id: string) => void;
  onViewDetails?: (id: string) => void; // Añadimos esta prop
}
  ...
  return (
    <Card className="overflow-hidden transition-all duration-300 hover:shadow-lg group">
      ...
      <CardContent className="pt-4">
        <h3 className="font-bold text-lg mb-1 text-secondary line-clamp-1">{property.title}</h3>
        ...
        <div className="flex items-center text-gray-500 text-sm mb-3">
          ...
          <span className="line-clamp-1">{property.location}, London {property.postcode}</span>
        </div>
